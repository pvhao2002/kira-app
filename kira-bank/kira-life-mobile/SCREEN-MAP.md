...
Tên mockup dưới đây là thư mục trong `../docs/mobile/Design UI UX`. Route có tham số sử dụng ID của dữ liệu ban đầu; có
thể mở trực tiếp trên web hoặc đi từ nút trong app.
...
| 25 | ch_nh_s_a_h_s                        | `/profile-settings`                           | Chỉnh sửa hồ sơ thật                                        |
| 26 | trung_t_m_th_ng_b_o                  | `/notifications`                              | Chuông thông báo                                            |
| 27 | t_o_h_s_tra_so_t                     | `/investment-report-create`                   | Báo cáo sai lệch transaction thật                           |
| 28 | danh_s_ch_h_s_tra_so_t               | `/investment-reports`                         | Hồ sơ tra soát của tôi                                      |
| 29 | chi_ti_t_h_s_tra_so_t                | `/investment-report-detail?id=1`              | Chi tiết và lịch sử bất biến                                |
| 30 | qu_n_tr_h_s_tra_so_t                 | `/admin-investment-reports`                   | Admin xử lý hồ sơ                                           |
| 31 | qu_n_l_cloudflare_r2                 | `/admin-cloudflare`                           | Admin quản lý Workers AI/R2                                 |
| 32 | qu_n_tr_queue_ai                     | `/admin-ai-queue`                             | Admin theo dõi queue toàn hệ thống                          |
| 33 | chi_ti_t_queue_ai                    | `/ai-job-detail?id=1`                         | Người dùng xem timeline lifecycle AI                        |
| 34 | chi_ti_t_queue_ai_admin              | `/admin-ai-job-detail?id=1`                   | Admin xem timeline lifecycle AI toàn hệ thống               |
| 35 | trung_t_m_s_c_kh_e                   | `/health`                                     | Tab Sức khỏe: dashboard BMI, năng lượng và thống kê         |
| 36 | h_s_s_c_kh_e                         | `/health-profile`                             | Khai báo hồ sơ, mục tiêu và giới hạn an toàn                |
...
Các trạng thái trên đã được dựng bằng thành phần React Native, với ảnh minh họa riêng lẻ lấy từ HTML và biểu đồ SVG
native. `screen.png` không được dùng làm nền nguyên màn.

- Header Kira Bank, biểu tượng, avatar, ba tab và các thanh tác vụ đã cập nhật theo từng nhóm màn.
- Các màn sửa bản ghi, cách xử lý và báo cáo sử dụng bề mặt dạng sheet; bộ lọc là modal có vùng cuộn và nút áp dụng cố
  định.
- Công cụ chọn kịch bản nằm tại `/demo`, đã tách khỏi màn Cá nhân. Chọn nhóm **Mockup** để xem số liệu lịch sử, kết quả
  xác nhận và thống kê tương ứng ảnh nguồn.
- Dữ liệu tương tác đang lưu trên thiết bị được giữ khi nâng cấu trúc bộ lọc. Điều chỉnh hạn mức thẻ và hạn mức ngân
  hàng có phạm vi riêng, kèm lịch sử lý do cục bộ.

Bằng chứng và danh sách kiểm tra hiện tại: [VISUAL-QA.md](VISUAL-QA.md). Những ghi nhận kiểm tra của phiên bản trước đã
được thay bằng kết quả của phiên bản hiện tại.

Không coi việc đủ route hoặc không lỗi JavaScript là chứng nhận giống hệt từng pixel. Bản native trên iPhone/Android
thật, bàn phím hệ thống, phóng to chữ và hiệu ứng kính của thiết bị chưa được kiểm chứng trong lượt này.
