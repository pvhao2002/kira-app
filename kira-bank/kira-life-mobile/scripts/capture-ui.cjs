...
async function seedScenario(page, name) {
  ...
  const labels = {
    'reference-history': 'Mockup · Lịch sử & chi tiết',
    'reference-account': 'Mockup · Thống kê tài khoản',
    'reference-partial': 'Mockup · Kết quả một phần',
    'reference-success': 'Mockup · Kết quả toàn bộ',
    'reference-filter': 'Mockup · Bộ lọc',
    empty: 'Lịch sử trống'
  };
...
(async () => {
  ...
  for (const [name, route, referenceWidth, scenario] of routes) {
    ...
    for (const width of process.argv.includes('--all-widths') ? [...new Set([referenceWidth, 360, 390, 430])] : [referenceWidth]) {
      ...
      const clipping = await page.evaluate(() => [...document.querySelectorAll('[role=button]')].filter(e => {
        const r = e.getBoundingClientRect();
        return r.width > 0 && (r.right > innerWidth + 1 || r.left < -1) && !e.closest('[style*=overflow]');
      }).map(e => e.textContent));
      await page.evaluate(() => {
        for (const e of document.querySelectorAll('div')) if (e.scrollHeight > e.clientHeight + 50 && getComputedStyle(e).overflowY === 'auto') e.scrollTop = e.scrollHeight;
      });
      await page.waitForTimeout(150);
      await page.screenshot({path: path.join(out, `${name}-${width}-bottom.png`)});
      await page.evaluate(() => {
        for (const e of document.querySelectorAll('div')) if (e.scrollTop) e.scrollTop = 0;
      });
