...
export function DemoProvider({children}: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem(KEY).then(raw => {
      ...
      if (mounted) setStorageError('Không đọc được dữ liệu đã lưu. Đang dùng dữ liệu mẫu ban đầu.');
    })
      .finally(() => {
        if (mounted) {
  ...
  function confirm() {
    ...
    update(s => {
      ...
      const drafts = s.drafts.map(d => {
        ...
        if (!s.jobs.some(j => j.id === d.source && j.status === 'ready')) {
          result.unresolved++;
          return d;
        }
        if (d.decision === 'skip') {
        ...
        if (existing >= 0 && d.decision !== 'merge') {
          ...
          result.unresolved++;
          return {...d, conflict: true};
        }
        if (d.conflict && d.decision === 'accept') {
          result.unresolved++;
          return d;
        }
        if (d.decision === 'merge') {
      ...
      const jobs = s.jobs.map(j => drafts.some(d => d.source === j.id) && drafts.filter(d => d.source === j.id).every(d => d.saved) ? {
        ...j,
  ...
  function scenario(name: string) {
    update(() => {
      ...
      if (name.startsWith('reference-')) {
        s.accounts[0] = {
          ...s.accounts[0],
          username: 'ssi_trietnguyen',
          email: 'triet.nguyen@kirabank.vn',
          phone: '0988 123 456'
        };
        s.transactions = [
          {
            ...
            externalId: 'MB-FT2410189821',
            description: 'Nạp tiền ký quỹ tài khoản chứng khoán SSI đợt 2 tháng 10',
            status: 'done',
            source: 'job-1'
          },
          ...
          {...s.transactions[2], amount: 75000000, date: '2024-10-15T08:12'},
          {
            ...
            type: 'bonus',
            amount: 1000000,
            date: '2024-10-02T11:00',
            externalId: 'PROMO-FALL24',
            description: 'Thưởng chiến dịch mùa thu',
            status: 'done'
          },
        ];
        s.reports = [
          {
            ...
            reason: 'Nạp tiền vào SSI Pro Trader',
            detail: 'Điều tra viên: Chuyên viên Kira Bank đang đối chiếu biên lai Napas với cổng đối soát SSI. Dự kiến hoàn tất trong 15 phút.',
            date: '2024-10-18',
            status: 'reviewing'
          },
          {
            ...
            reason: 'Rút tiền về VNDIRECT D-Stock',
            detail: 'Cần bạn bổ sung ảnh chụp sao kê tài khoản ngân hàng thụ hưởng.',
            date: '2024-10-15',
            status: 'pending'
          },
          {
            id: 'REP-20241010-019',
            transactionId: 'tx-vcf',
            reason: 'Nạp Quỹ mở VinaCapital VESAF',
            detail: 'Đã hủy bỏ bản ghi trùng lặp. Sổ đối soát độc lập đã khớp hoàn toàn 100%.',
            date: '2024-10-10',
            status: 'resolved'
          },
        ];
        if (name === 'reference-empty') s.transactions = [];
        if (name === 'reference-filter') s.filter = {
          ...s.filter,
          accountId: 'ssi',
          types: ['deposit', 'bonus'],
          from: '2024-10-01',
      ...
      if (name === 'success') {
        s.drafts = s.drafts.map(d => ({...d, conflict: false}));
        s.transactions = s.transactions.filter(t => t.id !== 'tx-existing');
      }
  ...
  const value: Store = {
    ...
    saveAccount: a => update(s => ({
      ...s,
      accounts: s.accounts.some(x => x.id === a.id) ? s.accounts.map(x => x.id === a.id ? a : x) : [...s.accounts, a]
    })),
    editDraft: (id, patch) => update(s => ({
      ...s,
      drafts: s.drafts.map(d => d.id === id && !d.saved ? {...d, ...patch, id: d.id} : d)
    })),
    ...
    report: report => {
      ...
      update(s => ({
        ...s,
        reports: [{...report, id, date: new Date().toISOString().slice(0, 10), status: 'pending'}, ...s.reports]
      }));
      return id;
    },
  };
...
export function useDemo() {
  ...
  if (!context) throw new Error('Missing DemoProvider');
  return context;
}
