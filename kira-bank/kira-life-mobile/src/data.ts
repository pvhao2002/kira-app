...
export type DemoState = {
  ...
  frozenCards: string[];
  creditConfig?: {
    banks: Record<string, { limit: number; debt: number }>;
    cards: Record<string, number>;
    audit: { target: string; reason: string; date: string }[]
  }
};
...
export const decisionNames: Record<Decision, string> = {
  accept: 'Chấp nhận',
  merge: 'Gộp với bản ghi có sẵn',
  new: 'Lưu như giao dịch mới',
  skip: 'Bỏ qua'
};
export const jobNames: Record<JobStatus, string> = {
  pending: 'Đang chờ',
  running: 'Đang xử lý',
  ready: 'Sẵn sàng',
  failed: 'Thất bại',
...
export const money = (value: number, currency = 'VND', lang: 'vi' | 'en' = 'vi') => new Intl.NumberFormat(lang === 'en' ? 'en-US' : 'vi-VN', {
  style: 'currency',
  currency,
...
export function seed(): DemoState {
  return {
    ...
    transactions: [
      {
        ...
        type: 'bonus',
        amount: 2500000,
        date: '2024-10-18T14:15',
        externalId: 'BONUS-001',
        description: 'Khoản thưởng quý III',
        status: 'done'
      },
      {
        id: 'tx-withdraw',
    ...
    drafts: [
      {
        ...
        description: 'Nạp tiền tài khoản SSI',
        status: 'done',
        decision: 'accept',
        conflict: false,
        selected: true,
    ...
    jobs: [
      ...
      {
        id: 'job-2',
        name: 'bien_lai_thuong_quy_02.jpg',
        size: '1.8 MB',
        status: 'running',
        attempts: 1,
        accountId: 'ssi'
    ...
    reports: [{
      ...
      reason: 'Sai số tiền',
      detail: 'Đề nghị kiểm tra chứng từ gốc.',
      date: '2024-10-19',
      status: 'reviewing'
    }],
  };
...
export function filteredTransactions(s: DemoState, lang: 'vi' | 'en' = 'vi') {
  const f = s.filter;
  return s.transactions.filter(t => (f.accountId === 'all' || t.accountId === f.accountId) && (!f.types.length || f.types.includes(t.type)) && (f.status === 'all' || t.status === f.status)
    ...
    && `${t.externalId} ${t.description} ${typeNames[t.type]}`.toLocaleLowerCase(lang).includes(f.query.toLocaleLowerCase(lang)))
