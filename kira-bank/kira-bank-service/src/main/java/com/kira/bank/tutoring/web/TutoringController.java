...
public class TutoringController {
    ...
    @GetMapping("/week")
    WeekResponse week(@AuthenticationPrincipal Long user,
                      @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate weekStart) {
        return service.week(user, weekStart);
    }

    @GetMapping("/students")
    List<StudentResponse> students(@AuthenticationPrincipal Long user) {
    ...
    StudentResponse createStudent(
        @AuthenticationPrincipal Long user, @Valid @RequestBody StudentRequest request) {
        return service.createStudent(user, request);
    }

    @PutMapping("/students/{id}")
    StudentResponse updateStudent(@AuthenticationPrincipal Long user, @PathVariable Long id,
                                  @Valid @RequestBody StudentRequest request) {
        return service.updateStudent(user, id, request);
    }
    ...
    @PostMapping("/series")
    @ResponseStatus(HttpStatus.CREATED)
    SeriesResponse createSeries(
        @AuthenticationPrincipal Long user, @Valid @RequestBody SeriesRequest request) {
        return service.createSeries(user, request);
    }

    @PutMapping("/series/{id}")
    SeriesResponse updateSeries(@AuthenticationPrincipal Long user, @PathVariable Long id,
    ...
    @PutMapping("/series/{seriesId}/occurrences/{occurrenceDate}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void saveException(@AuthenticationPrincipal Long user, @PathVariable Long seriesId,
        ...
        service.saveException(user, seriesId, occurrenceDate, request);
    }

    @DeleteMapping("/series/{seriesId}/occurrences/{occurrenceDate}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void restoreException(@AuthenticationPrincipal Long user, @PathVariable Long seriesId,
                          @PathVariable @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate occurrenceDate,
                          @Valid @RequestBody RestoreExceptionRequest request) {
        service.restoreException(user, seriesId, occurrenceDate, request);
    }
}
