...
public class LodgingController {
    ...
    @GetMapping
    PageResponse<ListingResponse> page(@AuthenticationPrincipal Long user, @RequestParam(defaultValue = "") String search, @PageableDefault(size = 20) Pageable pageable) {
        Page<ListingResponse> page = service.page(user, search, pageable);
        return new PageResponse<>(page.getContent(), new PageMeta(page.getNumber(), page.getSize(), page.getTotalElements(), page.getTotalPages()));
    }
    ...
    @GetMapping("/{id}")
    ListingResponse detail(@AuthenticationPrincipal Long user, @PathVariable Long id) {
    ...
    void delete(@AuthenticationPrincipal Long user, @PathVariable Long id) {
        service.delete(user, id);
    }

    @PostMapping("/{id}/distances/recalculate")
    ListingResponse recalculate(@AuthenticationPrincipal Long user, @PathVariable Long id) {
        service.tryRecalculate(user, id);
        return service.detail(user, id);
    }
    ...
    @ResponseStatus(HttpStatus.CREATED)
    ImageResponse upload(@AuthenticationPrincipal Long user, @PathVariable Long id, @RequestPart("file") MultipartFile file) throws IOException {
        return service.uploadImage(user, id, file);
    }

    @DeleteMapping("/{id}/images/{attachmentId}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void deleteImage(@AuthenticationPrincipal Long user, @PathVariable Long id, @PathVariable Long attachmentId) {
        service.deleteImage(user, id, attachmentId);
    }

    @GetMapping("/{id}/images/{attachmentId}/content")
    ResponseEntity<byte[]> content(@AuthenticationPrincipal Long user, @PathVariable Long id, @PathVariable Long attachmentId) {
        Attachment attachment = service.content(user, id, attachmentId);
        byte[] bytes = storage.download(attachment.getR2AccountId(), attachment.getStorageKey());
        return ResponseEntity.ok().contentType(MediaType.parseMediaType(attachment.getMimeType())).header(HttpHeaders.CONTENT_DISPOSITION, ContentDisposition.inline().filename(attachment.getOriginalName(), StandardCharsets.UTF_8).build().toString()).body(bytes);
    }

    @PutMapping("/{id}/reviews/me")
    ReviewResponse review(@AuthenticationPrincipal Long user, @PathVariable Long id, @Valid @RequestBody ReviewRequest request) {
        return service.review(user, id, request);
    }

    @GetMapping("/{id}/reviews")
    List<ReviewResponse> reviews(@AuthenticationPrincipal Long user, @PathVariable Long id) {
        return service.reviews(user, id);
    }

    @GetMapping("/reference-locations")
    List<ReferenceLocationResponse> locations(@AuthenticationPrincipal Long user) {
        return service.locations(user);
    }

    @GetMapping("/address-suggestions")
    List<AddressSuggestionResponse> addressSuggestions(@RequestParam String q) {
        return service.addressSuggestions(q);
    }

    @PostMapping("/reference-locations")
    @ResponseStatus(HttpStatus.CREATED)
    ...
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void deleteLocation(@AuthenticationPrincipal Long user, @PathVariable Long id) {
        service.deleteLocation(user, id);
    }
