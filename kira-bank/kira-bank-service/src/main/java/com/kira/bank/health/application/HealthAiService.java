...
public class HealthAiService {
    ...
    private static final String PROMPT="""
        ...
        """;
    public List<AiJob> jobs(long user) { return repo.jobs(user); }
    public AiJob generate(long user,AiRequest request) {
        health.validate(request); var profile=health.requireProfile(user); var p=profile.data();
        if(request.weekStart().getDayOfWeek()!=DayOfWeek.MONDAY || request.fromDate().isBefore(request.weekStart()) || request.fromDate().isAfter(request.weekStart().plusDays(6))
            || request.fromDate().isBefore(LocalDate.now(health.zone(p.timezone())))) throw HealthService.invalid("HEALTH_AI_DATE_INVALID");
        ...
        transactions.executeWithoutResult(status->{
            ...
            if(repo.jobs(user).stream().anyMatch(j->j.status().equals("RUNNING"))) throw new ApiException(HttpStatus.CONFLICT,"HEALTH_AI_ALREADY_RUNNING","A health plan is already being generated");
            repo.startJob(user,id);
        });
        try {
            // Omit identity, birth date, precise measurements and raw Watch records from the provider input.
            Map<String,Object> input=new LinkedHashMap<>();
            input.put("language",request.language()); input.put("fromDate",request.fromDate()); input.put("weekEnd",request.weekStart().plusDays(6));
            input.put("goal",p.goal()); input.put("calorieAdjustment",p.calorieAdjustment());
            input.put("baselineTarget",summary.restingEstimate()*p.activityFactor()+p.calorieAdjustment());
            input.put("foodPreferences",p.foodPreferences()); input.put("allergies",p.allergies()); input.put("avoidedFoods",p.avoidedFoods());
            input.put("preparationMinutes",p.preparationMinutes()); input.put("exerciseExperience",p.exerciseExperience()); input.put("equipment",p.equipment());
            input.put("availability",p.availability()); input.put("movementRestrictions",p.movementRestrictions());
            var response=client.generate(PROMPT,input);
            if(!response.path("conflicts").isArray() || !response.path("conflicts").isEmpty()) throw HealthService.invalid("HEALTH_PLAN_RESTRICTION_CONFLICT");
            var copy=response.deepCopy(); ((com.fasterxml.jackson.databind.node.ObjectNode)copy).remove("conflicts");
            PlanData data=mapper.treeToValue(copy,PlanData.class); health.validatePlan(p,request.weekStart(),data);
            for(var item:data.items()) if(item.date().isBefore(request.fromDate()) || item.completed()) throw HealthService.invalid("HEALTH_AI_INVALID_RESPONSE");
            for(LocalDate day=request.fromDate();!day.isAfter(request.weekStart().plusDays(6));day=day.plusDays(1)) {
                LocalDate date=day;
                if(data.items().stream().noneMatch(i->i.date().equals(date)&&i.kind().equals("MEAL")) || data.items().stream().noneMatch(i->i.date().equals(date)&&!i.kind().equals("MEAL"))) throw HealthService.invalid("HEALTH_AI_INVALID_RESPONSE");
            }
            ...
            if(!review.path("conflicts").isArray()||!review.path("conflicts").isEmpty()) throw HealthService.invalid("HEALTH_PLAN_RESTRICTION_CONFLICT");
            PlanData finalData=data;
            return transactions.execute(status->{
                repo.lockUser(user);
                if(health.requireProfile(user).version()!=profile.version()) throw new ApiException(HttpStatus.CONFLICT,"HEALTH_PROFILE_CHANGED","Profile changed while generating");
                ...
                if(active!=null) items.addAll(active.data().items().stream().filter(i->i.date().isBefore(request.fromDate())).toList());
                ...
                repo.finishJob(user,id,"READY",draft.id(),null); return new AiJob(id,"READY",draft.id(),null);
            });
        } catch(Exception e) {
            String code=e instanceof ApiException a?a.getCode():"HEALTH_AI_UNAVAILABLE";
            repo.finishJob(user,id,"FAILED",null,code); return new AiJob(id,"FAILED",null,code);
        }
    }
}
