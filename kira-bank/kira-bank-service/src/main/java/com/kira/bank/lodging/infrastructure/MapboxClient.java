...
public class MapboxClient {
    ...
    public Point geocode(String address) {
        ...
        try {
            ...
            if (response == null || response.features == null || response.features.isEmpty())
                throw unavailable("MAPBOX_ADDRESS_NOT_FOUND");
            ...
            logProviderFailure("geocode", ex);
            throw unavailable("MAPBOX_UNAVAILABLE");
        }
    }
    ...
    public List<AddressSuggestion> suggest(String query) {
        ...
        try {
            ...
            logProviderFailure("autocomplete", ex);
            throw unavailable("MAPBOX_UNAVAILABLE");
        }
    }
    ...
    private void requireToken() {
        if (properties.accessToken() == null || properties.accessToken().isBlank()) {
            log.warn("Mapbox request skipped code=MAPBOX_NOT_CONFIGURED");
            throw unavailable("MAPBOX_NOT_CONFIGURED");
        }
    }
    ...
    private ApiException unavailable(String code) {
        return new ApiException(HttpStatus.SERVICE_UNAVAILABLE, code, "Không thể tính khoảng cách lúc này");
    }
    ...
    private String destinationIndexes(int count) {
        return java.util.stream.IntStream.rangeClosed(1, count).mapToObj(String::valueOf).collect(java.util.stream.Collectors.joining(";"));
    }

    private String label(Feature feature) {
        if (feature.properties != null && feature.properties.fullAddress != null && !feature.properties.fullAddress.isBlank())
    ...
    @JsonIgnoreProperties(ignoreUnknown = true)
    record Feature(String id, String name, @JsonProperty("place_formatted") String placeFormatted,
    ...
    @JsonIgnoreProperties(ignoreUnknown = true)
    record Properties(@JsonProperty("full_address") String fullAddress, Context context) {
    }

    @JsonIgnoreProperties(ignoreUnknown = true)
    record Context(Region region) {
    }
    ...
    @JsonIgnoreProperties(ignoreUnknown = true)
    record MatrixResponse(List<List<Double>> distances) {
    }
}
