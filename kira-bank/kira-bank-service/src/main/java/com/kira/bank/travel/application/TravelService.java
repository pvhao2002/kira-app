...
public class TravelService {
    ...
    private static ApiException invalid(String message) { return new ApiException(HttpStatus.BAD_REQUEST,"TRAVEL_INVALID",message); }
    private static ApiException missing() { return new ApiException(HttpStatus.NOT_FOUND,"TRAVEL_NOT_FOUND","Travel record not found"); }
    private static void changed(int count) {
        if (count != 1) throw new ApiException(HttpStatus.CONFLICT,"TRAVEL_VERSION_CONFLICT","Reload this trip before saving again");
    }
    ...
    public List<TripView> list(long user) { return repo.list(user).stream().map(this::view).toList(); }
    public TripView get(long user, String id) { return view(require(user,id,false)); }
    private TripView view(Trip trip) { return new TripView(trip.id(),trip.data(),trip.version(),summary(trip.data())); }
    ...
    private long minor(BigDecimal value, int scale) {
        ...
        catch (ArithmeticException e) { throw invalid("Amount has invalid currency precision"); }
    }
    private void validate(TripData data) {
        ...
        try { ZoneId.of(data.timezone()); } catch (DateTimeException e) { throw invalid("Invalid timezone"); }
        ...
        for (var booking : data.bookings()) {
            if (!booking.url().isBlank()) {
                ...
                } catch (IllegalArgumentException e) { throw invalid("Invalid booking link"); }
            }
        }
    }
    ...
    public Summary summary(TripData data) {
        ...
        data.members().forEach(m -> { paid.put(m.id(),0L); owed.put(m.id(),0L); });
        long total=0;
        for (var expense:data.expenses()) {
            long amount=minor(expense.amount(),scale); total+=amount;
        ...
        for (String debtor:net.keySet()) {
            ...
            for (String creditor:net.keySet()) {
                ...
                net.put(debtor,net.get(debtor)+amount); net.put(creditor,net.get(creditor)-amount);
            }
        }
        return new Summary(BigDecimal.valueOf(total,scale),balances,transfers);
    }
    public List<FileView> files(long user,String trip) { require(user,trip,false); return repo.files(user,trip); }
    @Transactional
    public FileView upload(long user,String trip,MultipartFile file) {
        ...
        byte[] bytes;
        try { bytes=file.getBytes(); } catch (java.io.IOException e) { throw invalid("Could not read file"); }
        String type;
        if (bytes.length>=5 && bytes[0]=='%' && bytes[1]=='P' && bytes[2]=='D' && bytes[3]=='F' && bytes[4]=='-') type="application/pdf";
        else if (bytes.length>=8 && (bytes[0]&255)==137 && bytes[1]==80 && bytes[2]==78 && bytes[3]==71 && bytes[4]==13 && bytes[5]==10 && bytes[6]==26 && bytes[7]==10) type="image/png";
        else if (bytes.length>=3 && (bytes[0]&255)==255 && (bytes[1]&255)==216 && (bytes[2]&255)==255) type="image/jpeg";
    ...
    public FileContent download(long user,String trip,String id) {
        var file=repo.file(user,trip,id); if(file==null) throw missing(); return file;
    }
    @Transactional
    public void deleteFile(long user,String trip,String id) {
        require(user,trip,true); if(repo.deleteFile(user,trip,id)!=1) throw missing();
    }
}
