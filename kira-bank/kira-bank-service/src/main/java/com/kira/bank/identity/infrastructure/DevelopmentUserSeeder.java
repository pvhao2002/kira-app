...
public class DevelopmentUserSeeder implements ApplicationRunner {
    ...
    @Value("${app.seed-development-users:false}")
    boolean enabled;
    ...
    public void run(ApplicationArguments args) {
        if (!enabled) return;
        create("admin@kira.local", "KiraAdmin123!", "Quản trị Kira", true);
        create("user@kira.local", "KiraUser123!", "Người dùng Kira", false);
    }
    ...
    private void create(String email, String password, String name, boolean admin) {
        ...
        if (admin) u.getRoles().add(roles.findByName("ROLE_ADMIN").orElseThrow());
        users.save(u);
    }
}
