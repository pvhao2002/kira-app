...
public final class TravelDtos {
    ...
    public record ShoppingItem(@NotBlank @Size(max = 36) String id, @NotBlank @Size(max = 160) String name,
                               @NotBlank @Size(max = 80) String category, @Min(1) @Max(999) int quantity,
                               boolean purchased) {
    }

    public record PreparationItem(@NotBlank @Size(max = 36) String id, @NotBlank @Size(max = 160) String title,
                                  @NotNull @Size(max = 2000) String notes, LocalDate dueDate, boolean completed) {
    }

    public record ChecklistItem(@NotBlank @Size(max = 36) String id, @NotBlank @Size(max = 160) String title,
                                @NotNull @Size(max = 80) String category, boolean completed) {
    }

    public record Expense(@NotBlank @Size(max = 36) String id, @NotBlank @Size(max = 160) String title,
                          @NotNull LocalDate date,
                          @NotNull @DecimalMin("0.01") @DecimalMax("1000000000000") BigDecimal amount,
    ...
    public record TripData(@NotBlank @Size(max = 160) String name, @NotBlank @Size(max = 200) String destination,
                           ...
                           @NotNull @Size(max = 500) List<@NotNull @Valid ShoppingItem> shopping,
                           @NotNull @Size(max = 500) List<@NotNull @Valid PreparationItem> preparations,
                           @NotNull @Size(max = 500) List<@NotNull @Valid ChecklistItem> checklist,
