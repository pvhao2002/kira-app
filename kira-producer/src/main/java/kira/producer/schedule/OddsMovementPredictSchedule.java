...
@Service
@Log
@ConditionalOnProperty(name = "kira.producer.predict-schedule.odds-movement-enabled", havingValue = "true", matchIfMissing = true)
public class OddsMovementPredictSchedule {
            ...
            where cpq.queue_key is null
              ...
              and exists (select 1
                          ...
                          where o.event_id = e.event_id
                            ...
                            and o.line <> ''
                            and o.price_a is not null
                            and o.price_b is not null)
              and exists (select 1
                          from event_odds o
                          where o.event_id = e.event_id
                            and o.market = 'hdc'
    ...
    public OddsMovementPredictSchedule(
    ...
    @Scheduled(fixedDelay = 15, initialDelay = 3, timeUnit = TimeUnit.MINUTES)
    public void enqueueOddsMovementPredictions() {
        if (queueBackpressureService.isQueueOverLimit(RabbitMQConfig.QUEUE_PREDICTION, QUEUE_MAX_MESSAGES)) {
            log.log(Level.INFO, "Skip Odds Movement predict scheduling: prediction queue over limit");
            return;
        }

        List<Long> eventIds = jdbcTemplate.query(
                SQL_SELECT_EVENTS,
                Map.of("queue_type", PredictionQueueTypes.PREDICT_ODDS_MOVEMENT),
        ...
        log.log(Level.INFO, "Enqueueing Odds Movement predictions for {0} events", eventIds.size());
        ...
        var params = eventIds.stream()
                .map(id -> new MapSqlParameterSource("queue_key", String.valueOf(id))
                        .addValue("queue_type", PredictionQueueTypes.PREDICT_ODDS_MOVEMENT))
