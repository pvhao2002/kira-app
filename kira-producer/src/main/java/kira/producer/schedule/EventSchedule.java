...
public class EventSchedule {
    ...
    public void crawlOddForUpcomingEvent() {
        var result = jdbcTemplate.query(SQL_GET_EVENT_UPCOMING,
                Map.of("queue_type", CrawlQueueConstants.CRAWL_UPCOMING_EVENT),
        ...
        var params = result.stream()
                .map(it -> new MapSqlParameterSource("queue_key", it)
                        .addValue("queue_type", CrawlQueueConstants.CRAWL_UPCOMING_EVENT)
    ...
    public void event() {
        ...
        log.info("kira-producer >> Scheduled crawl odd for event analyst, total: " + result.size());
    }
}
