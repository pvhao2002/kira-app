...
public class Bet {
    ...
    public MapSqlParameterSource toParamPredict(Event evt) {
        ...
        oddsHandicap.stream()
                ...
                .ifPresentOrElse(e -> {
                    ...
                    param.addValue("first_hdc", e.getHome().split(" ")[0] + "#" + e.getAway().split(" ")[0]);
                }, () -> {
                    param.addValue("first_home_odds", null);
                    param.addValue("first_away_odds", null);
                    param.addValue("first_hdc", null);
                });
        ...
        oddsHandicap.stream()
                ...
                .ifPresentOrElse(e -> {
                    ...
                    param.addValue("last_hdc", e.getHome().split(" ")[0] + "#" + e.getAway().split(" ")[0]);
                }, () -> {
                    param.addValue("last_home_odds", null);
                    param.addValue("last_away_odds", null);
                    param.addValue("last_hdc", null);
                });

        // ----- Corner -----
        var oddsCorner = getOddsCorner().stream()
                .filter(odd -> DateUtil.parseOddDate(odd.getOddDate(), null) != null)
                .toList();
        ...
        oddsCorner.stream()
                .min(Comparator.comparing(o -> DateUtil.parseOddDate(o.getOddDate(), null)))
                .ifPresentOrElse(odd -> {
                    param.addValue("first_over_corner_odds", odd.getOver());
                    param.addValue("first_under_corner_odds", odd.getUnder());
                    param.addValue("first_corner", odd.getCorner());
                }, () -> {
                    param.addValue("first_over_corner_odds", null);
                    param.addValue("first_under_corner_odds", null);
                    param.addValue("first_corner", null);
                });
        ...
        oddsCorner.stream()
                .max(Comparator.comparing(o -> DateUtil.parseOddDate(o.getOddDate(), null)))
                .ifPresentOrElse(odd -> {
                    param.addValue("last_over_corner_odds", odd.getOver());
                    param.addValue("last_under_corner_odds", odd.getUnder());
                    param.addValue("last_corner", odd.getCorner());
                }, () -> {
                    param.addValue("last_over_corner_odds", null);
                    param.addValue("last_under_corner_odds", null);
                    param.addValue("last_corner", null);
                });
    ...
    public MapSqlParameterSource toPram(long eventId) {
        var param = new MapSqlParameterSource("event_id", eventId);
        ...
        var oddConer = getOddsCorner().stream()
                .filter(odd -> DateUtil.parseOddDate(odd.getOddDate(), null) != null)
                .toList();
        ...
        OddAnalyst minOddCorner = oddConer.stream()
                .min(Comparator.comparing(o -> DateUtil.parseOddDate(o.getOddDate(), null)))
                .map(e -> OddAnalyst.builder()
                        .line(e.getCorner())
                        .overOdd(OddConverter.parse(e.getOver()))
                        ...
                        .build())
                .orElse(new OddAnalyst());
        OddAnalyst maxOddCorner = oddConer.stream()
                .max(Comparator.comparing(o -> DateUtil.parseOddDate(o.getOddDate(), null)))
                .map(e -> OddAnalyst.builder()
                        .line(e.getCorner())
                        .overOdd(OddConverter.parse(e.getOver()))
        ...
        param.addValue("first_over_corner_odds", minOddCorner.getOverOdd());
        param.addValue("first_under_corner_odds", minOddCorner.getUnderOdd());
        param.addValue("last_over_corner_odds", maxOddCorner.getOverOdd());
        param.addValue("last_under_corner_odds", maxOddCorner.getUnderOdd());
        ...
        param.addValue("first_corner", minOddCorner.getLine());
        param.addValue("last_corner", maxOddCorner.getLine());
        return param;
    }
}
